...
const ProjectPageDisplay = async({ params }: { params: Promise<{ slug: string }> }) => {
  ...
  const readmeRaw = await fetchRepoReadme(slug).catch((e) => { console.log(e); return null });
  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-20">
      ...
      <section className="flex flex-col gap-3">
        ...
        {readmeContent && <section suppressHydrationWarning className="my-10 mx-0 md:m-10 mt-5 flex flex-col rounded-md border border-black/10 dark:border-white/10">
          <div className="p-3 text-sm line-clamp-1 justify-center text-zinc-600 font-medium font-mono overflow-x-auto whitespace-nowrap no-scrollbar">
            <a href={repo?.html_url + '/blob/main/README.md'} target="_blank" rel="noopener noreferrer" className="hover:underline">
              {`${repo?.full_name}/README.md`}
            </a>
          </div>
          <hr className='border-black/10 dark:border-white/10' />
          <MarkdownRenderer compiledHtml={readmeContent} 
      ...
      <section id="relevant-blog" className="my-8 flex flex-col gap-5">
        <h2 className="text-3xl sm:text-4xl font-bold text-zinc-800 dark:text-zinc-100 flex items-center line-clamp-1">
          ...
          <Link href="/blog" className="hover:text-blue-900 dark:hover:text-blue-300 transition-colors">
            Relevant Posts
          </Link>
        </h2>
